...
for gene in merged_gene_dict:
    ...
    for top4_score in top4_VBCs:
        top4_VBCs_gene_dict[gene][0].append(merged_gene_dict[gene][0][int(np.random.random()*len(merged_gene_dict[gene][2]))])
        top4_VBCs_gene_dict[gene][1].append(merged_gene_dict[gene][1][int(np.random.random()*len(merged_gene_dict[gene][2]))])
        top4_VBCs_gene_dict[gene][2].append(merged_gene_dict[gene][2][int(np.random.random()*len(merged_gene_dict[gene][2]))])
...
for i, item in enumerate(sorted(onlyGT_deltaVBC.items(), key=lambda t: t[1], reverse = True)):
    ...
    if i < hi_med:
        ...
        only_GT_locons.append(gene)
print(VBC_score_list[52])
print(VBC_score_list[104])
print(max(VBC_score_list))
print(min(VBC_score_list))
