...
data = 'Brunello_nuclear'
' + Species + '
UMI_data = pickle.load(open('/Users/georg.michlits/Desktop/Gen_data/v3_DICTIONARIES/CrUMI_2n_abs_LFC_d.sav','rb'))
...
Ms_Gene_d = pickle.load(open('/Users/georg.michlits/Desktop/Gen_properties/Ms/property_sav/Gene_d.sav', 'rb'))

Ms_hm_ortho = open('ms_hm_orthologs.txt','r')

Ms_to_Hm = {}
i = 0
for line in Ms_hm_ortho:
    if i > 0:
        col = line.rstrip('\n').split('\t')
        Msgene = col[2]
        Hmgene = col[4]
        Ms_to_Hm[Msgene] = Hmgene
    i += 1

gene_UMI_list = {}
for pos in UMI_data:
    Msgene = Ms_Gene_d[pos]
    if Msgene in Ms_to_Hm:
        gene_UMI_list[Ms_to_Hm[Msgene]] = 1
...
for pos_item in sorted(Screen_data.items(), key=lambda t: t[1]):
    pos = pos_item[0]
    LFC = pos_item[1]
    if Gene_d[pos] in gene_UMI_list:
        sum_total += 1
        if pos in CEG_d:
            sum_CEG += 1
        if pos in nonEG_d:
