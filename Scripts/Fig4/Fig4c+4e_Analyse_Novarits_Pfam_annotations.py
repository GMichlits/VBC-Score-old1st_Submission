...
for pos in RES16_LFC_d:
    if pos in AA_cons_d:
        if pos in inDelphi_infr_d:
            ...
            Enrico_score = model_T1_AAw5_6prop_abs_LFC.predict([data_list])[0]
            data_list.append(Enrico_score)
            gene = Gene_d[pos]
...
Feature_name_list.append('Enrico_score')
Target_name = 'Abs_LFC'
...
for gene in Structure_dict:
    #os.mkdir(gene+'_Rb')
    for n in range(len(Feature_name_list)):
        i = 0
        x_list = []
        y_list = []
        c_list = []
        for pos in sorted(Structure_dict[gene]):
            ...
            c_list.append(round(Data_list_dict[pos][n],4))

        name = 'DLD1' + Feature_name_list[n]
        sctr = plt.scatter(x_list, y_list, c=c_list, cmap='RdYlBu_r', linewidth=1)
        plt.colorbar(sctr)
        plt.title = name
        plt.xlabel('guides along coding DNA sequence')
        plt.ylabel('enrichment LFC')
        plt.savefig('properties'+'/' + gene + '_' + name + 'RB.pdf')
        plt.close()
